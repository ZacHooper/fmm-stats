...
def build_synthetic_compman_blob(
    ...
    # 36-byte HEADER
    hdr = bytearray(CM.HEADER_STRIDE)
...
def test_compman_honour_schema():
    ...
    buf = bytearray(CM.HONOUR_STRIDE)
    buf[0:5] = b"\xff" * 5
    ...
    rec = RD.read(bytes(buf), CM.HONOUR, 0)
    ...
    assert CM.HONOUR.span == CM.HONOUR_STRIDE == 55
