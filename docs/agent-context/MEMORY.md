...
- [Remote-agent SQL access](remote-duckdb-access.md) — PR #1 (open, unmerged): scrubbed DuckDB copy in R2, `ATTACH 's3://...' (READ_ONLY)` over httpfs using R2 creds directly (not the Worker — `*.workers.dev` is often unreachable from a restricted sandbox). Verified end-to-end; use `TYPE s3` + explicit `ENDPOINT` (not `TYPE r2`/`ACCOUNT_ID`, which mis-routes), and fetch the `httpfs` extension over HTTPS by hand if `INSTALL` 403s (it defaults to plain HTTP). Real store upload still pending.
