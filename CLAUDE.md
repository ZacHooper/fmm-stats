...
| Published | `site/` (static HTML + JSON) | **git** -> Cloudflare Pages | regenerated by `build_site.py`; see [`docs/DEPLOY.md`](docs/DEPLOY.md) |
...
# refreshing the phone-readable static site (after an import)
uv run python scripts/build_site.py                       # -> site/ (~7s); fails on a CA leak
...
- **Depth-chart logic lives in `dashboard/positions.py`**, shared by the Positions page and the
  static site. Change it there, not in either consumer, or the phone and the dashboard start
  giving different verdicts.
