...
| `stdt` | 1002 | id, dyom, mont, year, dyow, comp, nmmt, tvty, ndow, ntim | stage START date (dyom/mont/year/dyow) + TV scheduling + prize |
| `fxds` | 996 | id, mnsn, in_2, id_2, id_1, in_1, sbsn, type, nmmt, ofsd | fixture-set config (season, num matches, groups, seeding, team slots) |
| `endt` | 954 | id, dyom, mont, year, dyow, hidl, lwdl, wnty, time, type | stage END date + season-boundary flags (snst/snen) + winner info |
| `sdfd` | 928 | id, id_1, id_2, in_1, cash, posn, mnsn, dyow, in_2, comp | stage finance/date record: prize (cash) by finishing posn + stage dates |
| `nwdt` | 840 | year, dyow, dyom, mont, id, time, type, strl, in_1, nmmt | ?date |
| `nmsn` | 823 | id, stgn, stag, dyow, team, mnsn, crk, nmmt, in_2, type | per-team season schedule + rank (stage/team-slot/crk + TV scheduling) |
...
| `mnsn` | 466 | id, type, levl, ntms, year, stgn, Ttea, dyow, dyom, mont | season record (level, num teams, stages, team list via Ttea) |
| `nati` | 442 | id, Nnat, mnsn, year, type, dyow, dyom, mont, time, strl | nation's competition setup (Nnat + calendar + level) |
| `date` | 401 | id, mnsn, mtrl, nmlg, dyow, year, dyom, mont, nmxt, in_2 | date entity |
| `sbsn` | 345 | id, stgn, type, time, mnsn, strl, ntms, year, dyow, dyom | sub-season index |
| `drdt` | 341 | id, time, ddsd, mnsn, year, dyom, mont, dyow, in_2, type | draw date? |
| `rgdv` | 263 | id, comp, igmt, mxtm, semt, seed, GmTy, rats, DBID, rst1 | ?regional/relegation division config |
| `jcom` | 244 | id, comp, dyow, year, dyom, mont, time, przm, nmmt, cash | ?competition prize/schedule record |
| `rsid` | 230 | id, type, comp, vers, rsvt, utic, dyom, mont, fxri, stdt | ?rules/schedule record (comp + version) |
| `pare` | 226 | id, comp, igmt, umox, nxss, mntm, DBID, ntms, mxtm, type | parent competition (comp + team-count cfg) |
...
| `pvrn` | 213 | id, dyof, time, stgn, nmmt, mnsn, mtrl, type, ofsd, date | ?preview/previous-round record |
| `spst` | 208 | id, stad, przm, year, time, strl, dyow, dyom, mont, comp | stadium/sponsor prize record (stad+przm) |
| `accm` | 204 | id, type, sequ, gmsb, gmbl, tyar, prsw, sswn | ?achievement/accumulator (type+sequence) |
| `srci` | 198 | id, bnsc, decd, sfsr, mnsn, in_2, year, strl, type, time | ?season/source-comp record (bnsc/decd) |
| `rdci` | 190 | id, gmsb, rdcr, bnsc, type, cmty, valu, fxrl, usqn, tcpf | ?draw/registration counter (rdcr) |
...
| `dsbi` | 180 | id, stdt, bnsc, gmsb, accm, type, sequ, prsw, sswn, rdci | ?board/achievement record (bnsc + accm seq) |
| `wnpz` | 173 | id, cash, nmmt, tvty, curr, ndow, tvmt, ntim, crk, ofsd | winner prize by finishing position (crk) |
| `przm` | 172 | year, strl, dyom, mont, id, dyow, levl, cash, type, id_1 | prize money by level/position (levl + cash/przm amount) |
| `lcrl` | 154 | id, lcrg, igmt, mxtm, WrWt, type, GmTy, igma, semt, seed | ?rule record (lcrg ref) |
| `snrp` | 154 | id, sndr, type, cmty, valu, sdtr, mxss, NinE, usqn, tcpf | squad/registration rule (mxss max squad, NinE) |
| `lsdv` | 150 | id, comp, cash, dyow, curr, dyom, mont, year, strl, semt | league division setup (comp + prize + level) |
| `sred` | 142 | type, cmty, valu, sqsr, id, dyom, mont, year, sdnf, usqn | ?per-club squad rules (type=club tid, mxss=max squad) |
| `lspz` | 138 | id, cash, time, curr, mnsn, nmmt, stnm, type, ntms, ofsd | league-stage prize (cash by round/position) |
| `srsd` | 130 | id, dyom, mont, year, dyow | season reference date (dyom/mont/year) |
| `fnlc` | 127 | id, comp, dyow, year, strl, dyom, mont, time, aldt, type | ?final/calendar comp record |
| `dat2` | 124 | id, mnsn, mtrl, sbsn, type, time, team, ftac, stag, dyof | date entity 2 |
| `ncmp` | 124 | id, comp, semt, stag, seed, DBID, ntms, srnd, subr, type | nested/child competition (stage/seed/round) |
| `cash` | 112 | id, curr, type, WrWt, WrDl, lcrl, cmty, valu, trwi, usqn | cash amount |
| `tfrd` | 112 | type, cmty, valu, id, comp, styr, enyr, tcpf, sqsr, usqn | ?transfer/squad-rule record (type=club tid, styr..enyr window) |
...
| `apmn` | 98 | id, nmmt, cash, curr, tvty, tvmt, ndow, ntim, lgin, type | appearance / TV money (cash + TV scheduling) |
| `updy` | 94 | id, type, dyom, mont, cmty, valu, mnsn, time, in_2, year | ?dated update record (type/valu by date) |
...
| `trrl` | 62 | id, natr, frtj, cnem, lpcr, type, lnrl, plty, fcmn, valu | ?transfer/registration rule (natr/frtj/fcmn foreign cap) |
| `snda` | 60 | type, valu, cmty, id, mxss, tcpf, usqn, sqsr, sdnf, sqsv | squad rules by entity (mxss, valu) |
| `ofxd` | 60 | id, mnsn, in_2, sbsn, id_1, id_2, in_1 | ?fixture reference (mnsn/sbsn) |
| `nfxd` | 59 | przm, id, levl, mnsn, in_2, cash, id_1, id_2, ACfl, sbsn | ?prize-by-level fixture record (przm/cash) |
| `RLtm` | 59 | Ttea, id, seed, aqtp, tems, stag, type, year, ntms, indx | ranking/seeding team-list (Ttea members + seed/stage) |
| `CfOD` | 59 | przm, levl, id, year, dyom, mont, strl, id_2, id_1, dyow | ?competition-config-by-date |
| `CnRl` | 58 | type, cmty, valu, id, comp, styr, enyr, tcpf, Draf, sqsr | ?competition rule (entity, styr..enyr, Draf draft) |
| `SCsn` | 58 | id, type, ntms, stgn, stag, topp, btpl, seed, levl, time | stage config: ntms + topp(promoted)/btpl(relegated) + seed + level |
| `tppr` | 51 | id, nrpl, levl, przm, comp, stag, vsdp, indx, itvm, fnrg | ?promotion/prize record (nrpl) |
| `rfpr` | 51 | id, hgrk, comp, inac, type, tems, itvm, vsdp, fnrg, dyow | ?reputation/finish record (hgrk rank 7..27) |
| `city` | 50 | id, type, year, dyow, dyom, mont, strl, id_1, id_2, indx | city/location record (+ city id field) |
| `updv` | 48 | id, comp | ?update-division (comp ref) |
| `lwdv` | 48 | id, comp, tems, type, Bran, PrSt, vlys, mntm, mxtm, dyow | ?lower-division config (comp/tems) |
| `OPpr` | 48 | id, ntms, type, levl, lwrk, strl, year, dyow, dyom, mont | ?opponent/prize-pool (levl + lwrk rank) |
| `btpr` | 45 | id, nrpl | bottom places (relegation) variant |
| `PSfd` | 42 | id, mnsn, id_1, id_2 | **TBD** |
| `prmr` | 42 | id, nrpl | promotion places (nrpl count) — companion to relr |
| `TrSt` | 41 | stag, id, type, srcc, shsn, shnt, exlg, shpo, indx, Per% | ?stage/tier state |
| `pspl` | 37 | id, cash, curr, posn, type, ckch, ckpr, ckcc, ckrl, plyf | split? |
| `COdf` | 36 | id, team, crk, hgrk, lwrk, comp, nrpl, id_1, id_2, rank | competition squad rules (mnpl/mxpl) |
...
| `Nxdv` | 8 | id, DBID, ngps, comp, type, team, lfte, srcc, stag | ?next-division config |
...
| `mnsn` | 8263 | 0x11,0x12,0x0a | 0..240 | 76, 73, 6, 20, 145 | season record (level, num teams, stages, team list via Ttea) |
...
| `przm` | 3208 | 0x0a,0x0b,0x01,0x11,0x12 | 0..96809160 | 3, 5, 7, 2, 4737581 | prize money by level/position (levl + cash/przm amount) |
...
| `cmty` | 2033 | 0x11 | 1..255 | 255, 1 | 255-sentinel flag/?commentary |
...
| `stdt` | 1520 | 0x11,0x0a | 0..7 | 0, 4, 5, 1, 7 | stage START date (dyom/mont/year/dyow) + TV scheduling + prize |
| `dyof` | 1510 | 0x11 | 0..255 | 1, 248, 2, 255, 0 | day offset |
| `hgrk` | 1505 | 0x11 | 0..27 | 0, 4, 6, 13, 17 | season-highest rank |
| `info` | 1494 | 0x11,0x0b,0x0a,0x03,0x12 | 0..750 | 40, 90, 75, 70, 3 | counter/index value |
| `fxds` | 1494 | 0x0b,0x0a | 0..81 | 17, 3, 4, 21, 8 | fixture-set config (season, num matches, groups, seeding, team slots) |
| `mtrl` | 1422 | 0x02 | refs:    ×622,     ×294,    ×147,   ×78 | ->    , ->   , ->   , ->   , ->    | ?related-entity ref |
| `tems` | 1373 | 0x0b,0x11 | 0..80 | 3, 1, 0, 4, 2 | team count (stage) |
| `subr` | 1288 | 0x11 | 0..69 | 55, 30, 0, 20, 6 | sub-round |
| `semt` | 1274 | 0x11 | 0..9 | 1, 0, 2, 8, 9 | ?stage-element type (0..7) |
| `date` | 1233 | 0x0a | 2..8 | 5, 4, 3, 2, 6 | date entity |
| `styr` | 1202 | 0x12 | 2000..2027 | 2021, 2020, 2022, 2024, 2017 | start year |
| `nmsn` | 1159 | 0x0a,0x11,0x12 | 2..167 | 2, 3, 21, 108, 109 | per-team season schedule + rank (stage/team-slot/crk + TV scheduling) |
| `endt` | 1151 | 0x0a | 3..7 | 3, 5, 4, 6, 7 | stage END date + season-boundary flags (snst/snen) + winner info |
| `prio` | 1107 | 0x11 | 1..10 | 4, 2, 5, 6, 3 | priority (seeding) |
| `bsyo` | 1067 | 0x11 | 0..255 | 0, 1, 255, 2 | bool flag |
| `sdfd` | 1053 | 0x0a | 2..5 | 3, 4, 5, 2 | stage finance/date record: prize (cash) by finishing posn + stage dates |
| `vers` | 1017 | 0x11 | 0..6 | 1, 5, 4, 6, 3 | version |
| `lsdi` | 1017 | 0x11 | 0..44 | 10, 12, 20, 8, 0 | ?last-stage index |
| `drdt` | 1007 | 0x0a | 1..6 | 2, 3, 6, 4, 5 | draw date? |
| `nmlg` | 1004 | 0x11 | 1..2 | 2, 1 | num legs (1..2) |
| `tvds` | 991 | 0x0b | 0..28 | 12, 9, 7, 1, 0 | ?TV days (0..16) |
...
| `nrpl` | 820 | 0x11 | 0..14 | 1, 0, 6, 2, 3 | number of places (relegated / replays, context-dependent) |
| `nmxt` | 820 | 0x11,0x12 | 0..292 | 2, 4, 0, 1, 6 | num next matches (0..82) |
| `sqsr` | 808 | 0x0b,0x12 | 0..350 | 1, 3, 2, 0, 10 | squad-season ref |
| `sche` | 804 | 0x0b | 0..2 | 1, 0, 2 | scheduled flag |
| `posn` | 795 | 0x11 | 0..19 | 0, 1, 2, 3, 4 | finishing position (prize / table row) |
| `vlgr` | 770 | 0x0b,0x03 | 0..11 | 0, 2, 1, 4, 6 | ?(0..3) |
...
| `topp` | 675 | 0x11 | 0..60 | 0, 4, 3, 6, 2 | promoted places (top) |
...
| `ctmp` | 608 | 0x0b | 0..9 | 0, 3, 2, 4, 1 | ?count/template (0..9) |
...
| `rank` | 578 | 0x0b,0x03 | 0..28 | 18, 4, 2, 0, 12 | rank / seed value |
| `advs` | 577 | 0x0b | 0..3 | 1, 0, 3 | ?advantage (0..3) |
| `inac` | 543 | 0x03 | 0..1 | 0, 1 | inactive flag |
| `btpl` | 535 | 0x11 | 0..255 | 3, 7, 0, 11, 2 | relegated places (bottom) |
| `nati` | 529 | 0x0a,0x12,0x01 | 2..918748 | 2, 3, 769, 917496, 918740 | nation's competition setup (Nnat + calendar + level) |
| `nrds` | 511 | 0x11 | 0..5 | 4, 2, 3, 5, 1 | number of rounds |
| `rsvt` | 488 | 0x02,0x11 | refs:    ×20,    ×16,    !×6,    0×6 | ->   , 0, 1, 2, ->    | ?(0..6) |
| `ygap` | 487 | 0x11 | 1..100 | 4, 2, 100, 1 | ?year gap |
| `ind1` | 484 | 0x11 | 0..31 | 0, 5, 2, 1, 28 | index |
| `styo` | 482 | 0x11 | 0..255 | 1, 0, 255 | start-year flag |
| `chcl` | 482 | 0x11,0x03 | 0..7 | 1, 2, 3, 0, 5 | ?channel (0..7) |
...
| `dtrn` | 456 | 0x0a | 5..5 | 5 | date-range record (start/end month + year, e.g. transfer window) |
...
| `pvrn` | 420 | 0x11,0x0a,0x12 | 2..164 | 18, 6, 7, 2, 3 | ?preview/previous-round record |
...
| `spdO` | 410 | 0x11 | 0..255 | 0, 235, 245, 249, 1 | ?(0..255) |
...
| `decd` | 396 | 0x11 | 0..6 | 0, 1, 2, 5, 4 | ?decided (0..6) |
| `rkli` | 395 | 0x0b | 0..28 | 18, 12, 0, 11, 4 | ?rank-list size? |
| `gpid` | 395 | 0x11 | 0..15 | 0, 1, 2, 3, 4 | group id (0..15) |
| `ACfl` | 389 | 0x02 | refs:     ×266,    ×60,    ×19,    ×10 | ->   , ->    , ->  , ->   , -> @   | ?flag ref |
| `dat2` | 384 | 0x0a | 3..7 | 4, 5, 3, 6, 7 | date entity 2 |
| `brpd` | 378 | 0x0b | 0..5 | 1, 0, 2, 5, 3 | ?break period (0..5) |
| `sequ` | 372 | 0x11 | 0..45 | 38, 2, 14, 30, 6 | sequence no. |
| `idwi` | 370 | 0x11 | 1..7 | 7, 1, 2, 3, 4 | index |
| `ctin` | 367 | 0x11 | 0..8 | 0, 1, 2, 3, 4 | counter/index (0..7) |
...
| `Cdpc` | 358 | 0x0b | 0..8 | 0, 2, 1, 8, 3 | ?(0..8) |
| `ttac` | 349 | 0x03 | 0..1 | 0, 1 | bool flag |
| `pare` | 348 | 0x0a | 2..3 | 2, 3 | parent competition (comp + team-count cfg) |
| `SrTs` | 346 | 0x11 | 0..21 | 10, 0, 12, 7, 11 | ? |
| `itvm` | 342 | 0x0b | 0..22 | 1, 8, 0, 22, 19 | ?(0..8) |
| `fnrg` | 338 | 0x0b | 0..20 | 15, 1, 0, 20, 18 | ?finishing-position range (0..20) |
| `FxSt` | 337 | 0x0b | 0..7 | 0, 1, 2, 6, 3 | ?fixture state (0..7) |
| `ddsd` | 325 | 0x03 | 0..1 | 1, 0 | draw-seeded flag |
| `accm` | 324 | 0x0a | 3..4 | 4, 3 | ?achievement/accumulator (type+sequence) |
| `vsdp` | 320 | 0x0b | 0..1 | 1, 0 | bool flag |
| `MtTv` | 319 | 0x0b | 0..9 | 0, 1, 8, 2, 3 | ?match TV (0..9) |
| `hlps` | 318 | 0x0b | 0..5 | 0, 1, 3, 2, 4 | ?(0..5) |
| `relr` | 315 | 0x0a,0x11 | 1..7 | 3, 5, 4, 7, 2 | relegation places (nrpl count) |
| `mnpl` | 315 | 0x11 | 0..25 | 1, 2, 3, 4, 0 | min players (squad rule) |
| `stvd` | 314 | 0x0b | 0..12 | 5, 0, 9, 3, 12 | ?stage (0..12) |
| `lgto` | 314 | 0x0b | 0..1 | 0, 1 | ?league config flag |
| `crgt` | 314 | 0x0b | 0..68 | 0, 29, 28, 58, 55 | ?(0..68) |
| `srci` | 302 | 0x0a | 4..4 | 4 | ?season/source-comp record (bnsc/decd) |
| `sfsr` | 302 | 0x03 | 0..1 | 0, 1 | bool flag |
| `ldos` | 302 | 0x11 | 1..255 | 1, 255, 2, 4 | ?(1..255) |
| `rdci` | 300 | 0x0a | 3..4 | 3, 4 | ?draw/registration counter (rdcr) |
| `rdcr` | 300 | 0x11 | 0..9 | 8, 0, 2, 3, 6 | ?(0..9) |
| `sbty` | 300 | 0x11 | 1..33 | 1, 2, 27, 26, 12 | ?(1..17) |
| `ctll` | 299 | 0x0b | 0..32 | 0, 4, 8, 3, 16 | bool flag |
| `lgfx` | 298 | 0x0b | 0..2 | 1, 0, 2 | bool flag |
| `mxss` | 294 | 0x11 | 0..99 | 99, 30, 2, 1, 40 | max squad size |
| `dsbi` | 294 | 0x0a | 3..3 | 3 | ?board/achievement record (bnsc + accm seq) |
| `cnic` | 289 | 0x03 | 0..1 | 0, 1 | bool flag |
| `mxpl` | 289 | 0x11 | 0..25 | 1, 2, 3, 4, 5 | max players (squad rule) |
| `fxor` | 277 | 0x11 | 0..9 | 1, 0, 6, 8, 2 | fixture order |
| `rats` | 271 | 0x03 | 0..1 | 1, 0 | ratings flag |
| `fate` | 268 | 0x11 | 0..255 | 2, 0, 255, 4, 1 | ?(0..255, TrSt) |
| `GmTy` | 263 | 0x11 | 1..6 | 4, 5, 6, 1 | game type flag |
| `rgdv` | 263 | 0x0a | 2..3 | 2, 3 | ?regional/relegation division config |
| `rmic` | 257 | 0x03 | 0..1 | 1, 0 | bool flag |
| `qtsd` | 256 | 0x11 | 0..100 | 0, 90, 91, 92, 93 | ?quota (0..100) |
| `typz` | 255 | 0x0b | 0..49 | 16, 8, 14, 12, 20 | ?type (0..49) |
| `jcom` | 244 | 0x0a | 2..3 | 2, 3 | ?competition prize/schedule record |
| `wnty` | 240 | 0x11 | 1..39 | 7, 1, 9, 6, 20 | ?winner type/round |
...
| `quty` | 235 | 0x11 | 1..9 | 3, 2, 4, 1, 9 | ?quantity (1..9) |
| `stdm` | 234 | 0x11 | 1..31 | 1, 24, 31, 21, 4 | start day-of-month |
| `endm` | 234 | 0x11 | 1..31 | 31, 30, 29, 28, 20 | end day-of-month |
| `hcti` | 234 | 0x11 | 0..20 | 0, 5, 4, 3, 1 | ?home city index |
| `spst` | 231 | 0x0a | 2..3 | 2, 3 | stadium/sponsor prize record (stad+przm) |
| `rsid` | 230 | 0x0a | 2..3 | 2, 3 | ?rules/schedule record (comp + version) |
| `sths` | 230 | 0x03 | 0..1 | 1, 0 | bool flag |
| `rst1` | 226 | 0x11 | 0..18 | 0, 1, 10, 3, 12 | bool flag |
| `pots` | 224 | 0x11,0x0b | 1..30 | 6, 9, 5, 21, 10 | draw/seeding pots |
| `iqum` | 224 | 0x03 | 0..1 | 0, 1 | bool flag |
| `uhtt` | 224 | 0x11 | 0..1 | 0, 1 | bool flag |
| `bktm` | 223 | 0x12,0x11 | 0..2100 | 1945, 1400, 1300, 1330, 2030 | ?time (HHMM) |
| `wnpz` | 221 | 0x0a,0x01 | 2..88000 | 3, 2, 88000 | winner prize by finishing position (crk) |
| `stad` | 219 | 0x01,0x12 | 838..2000029204 | 2000029204, 68005347, 8826792, 980, 741569 | stadium (ref) |
| `qurl` | 219 | 0x0b,0x11 | 0..4 | 2, 1, 0, 3, 4 | qualification rule? |
| `sqsv` | 218 | 0x11 | 4..41 | 8, 28, 41, 25, 4 | squad-season value |
| `utic` | 218 | 0x03 | 0..1 | 0, 1 | bool flag |
| `Bktm` | 216 | 0x11 | 0..100 | 18, 20, 12, 28, 16 | bracket teams |
| `vlys` | 214 | 0x0b | 0..4 | 1, 2, 3, 4, 0 | ? |
| `frty` | 211 | 0x03,0x11 | 0..1 | 0, 1 | bool flag |
| `atin` | 210 | 0x11 | 0..17 | 1, 0, 2, 6, 3 | ?(0..17) |
| `stdw` | 210 | 0x11 | 1..7 | 7, 6, 1, 4 | start day-of-week |
| `endw` | 210 | 0x11 | 1..7 | 1, 7, 4 | end day-of-week |
| `sfar` | 208 | 0x11 | 1..10 | 2, 1, 3, 6, 10 | ?(1..10) |
| `edtv` | 205 | 0x11 | 0..4 | 2, 0, 1, 3, 4 | ?(0..4) |
| `mtdr` | 200 | 0x11 | 1..8 | 1, 8, 2 | ?match-day rule |
| `city` | 200 | 0x01,0x0a,0x12,0x11 | 2..74009628 | 100024, 102586, 102583, 74009628, 2 | city/location record (+ city id field) |
| `snrp` | 198 | 0x0a | 2..4 | 3, 2, 4 | squad/registration rule (mxss max squad, NinE) |
| `blpm` | 197 | 0x03 | 0..1 | 1, 0 | bool flag |
| `lspz` | 195 | 0x0a,0x12,0x01 | 2..52750 | 3, 2, 14000, 28000, 52750 | league-stage prize (cash by round/position) |
| `hdty` | 194 | 0x11 | 1..8 | 3, 4, 5, 8, 2 | ?head-to-head/seeding type |
| `dtfl` | 188 | 0x02 | refs:    ×46,     ×31,    ×30,    ×23 | ->   , ->   , ->  , ->   A, ->     | ?date-flag ref |
| `prmr` | 182 | 0x0a,0x11 | 2..6 | 5, 4, 3, 2, 6 | promotion places (nrpl count) — companion to relr |
| `lcrl` | 174 | 0x0a,0x01 | 2..156580 | 2, 156580, 3 | ?rule record (lcrg ref) |
| `apmn` | 174 | 0x0b,0x0a | 0..3 | 1, 0, 2, 3 | appearance / TV money (cash + TV scheduling) |
| `PrSt` | 173 | 0x0b,0x02 | refs:     ×100,    ×25,    ×3,    ×3 | 1, ->   , ->   , 2, ->     | ?flag |
| `rmnn` | 173 | 0x03 | 1..1 | 1 | const(=1) |
| `RdPr` | 173 | 0x11 | 0..0 | 0 | const(=0) |
| `TiTu` | 171 | 0x12 | 1230..2100 | 1730, 2000, 1330, 1400, 1800 | TV kickoff time slot (HHMM) |
| `sdnf` | 168 | 0x02 | refs:    ×38,   ×18,   ×16,    ×12 | ->   , ->   , ->  , ->   , ->À  | ?ref |
| `MlsY` | 168 | 0x12 | 2021..2027 | 2021, 2022, 2023, 2024, 2025 | milestone year (future projection) |
| `tmPL` | 167 | 0x0b | 1..30 | 1, 2, 3, 12, 4 | ?team placement |
| `ngps` | 164 | 0x11 | 1..16 | 2, 4, 6, 3, 8 | number of groups |
| `lsdv` | 163 | 0x0a | 2..3 | 2, 3 | league division setup (comp + prize + level) |
| `cmps` | 161 | 0x0b | 1..19 | 7, 6, 3, 4, 8 | number of component/child comps |
| `nmdt` | 160 | 0x11 | 2..46 | 20, 24, 22, 34, 18 | ?num dates |
| `srsd` | 160 | 0x0a | 4..6 | 5, 4, 6 | season reference date (dyom/mont/year) |
| `sred` | 160 | 0x0a | 4..5 | 5, 4 | ?per-club squad rules (type=club tid, mxss=max squad) |
...
| `RLtm` | 61 | 0x0a | 2..4 | 3, 4, 2 | ranking/seeding team-list (Ttea members + seed/stage) |
| `CfOD` | 59 | 0x0a | 3..5 | 4, 5, 3 | ?competition-config-by-date |
| `CnRl` | 58 | 0x0a | 1..2 | 2, 1 | ?competition rule (entity, styr..enyr, Draf draft) |
| `SCsn` | 58 | 0x0a | 2..2 | 2 | stage config: ntms + topp(promoted)/btpl(relegated) + seed + level |
| `cnem` | 56 | 0x11 | 5..12 | 12, 6, 11, 10, 5 | ?month-ish |
| `snfl` | 56 | 0x02 | refs:    ×8,   ×8,   ×8,   ×6 | ->   , ->  , ->   , ->  , ->   | **TBD** |
| `mage` | 56 | 0x03 | 1..1 | 1 | **TBD** |
| `TrSt` | 55 | 0x0a | 4..6 | 6, 4, 5 | ?stage/tier state |
...
| `rfpr` | 51 | 0x0a | 2..4 | 2, 4 | ?reputation/finish record (hgrk rank 7..27) |
| `TtUl` | 50 | 0x0b | 2..5 | 2, 3, 4, 5 | **TBD** |
| `DOmf` | 50 | 0x0b | 0..4 | 2, 3, 4, 1, 0 | **TBD** |
| `gsyo` | 48 | 0x03 | 0..1 | 1, 0 | **TBD** |
| `updv` | 48 | 0x0a | 2..2 | 2 | ?update-division (comp ref) |
| `lwdv` | 48 | 0x0a | 2..2 | 2 | ?lower-division config (comp/tems) |
| `OPpr` | 48 | 0x0a | 2..3 | 2, 3 | ?opponent/prize-pool (levl + lwrk rank) |
...
| `shnt` | 43 | 0x11 | 12..18 | 18, 12, 14, 16 | ?(12..14 foreign cap?) |
...
| `fcmn` | 40 | 0x11 | 1..17 | 16, 17, 1, 15 | ?foreign-player cap (15..17) |
...
| `tyar` | 30 | 0x03 | 0..1 | 1, 0 | bool flag |
...
| `shpo` | 25 | 0x11 | 0..10 | 0, 4, 6, 10, 8 | ?month |
...
| `NinE` | 24 | 0x03 | 0..1 | 0, 1 | bool flag |
