...
| Published (queryable) | `site-data/fm-<career>.duckdb`, ca/pa NULLed | **R2**, `/api/db` (Range-forwarding Worker route) | for a remote agent: `ATTACH 'https://.../api/db?career=<key>' (READ_ONLY)` over httpfs — arbitrary SQL, not just the fixed JSON shapes. Published by `scripts/publish_duckdb.py --upload`. NEVER git. |
