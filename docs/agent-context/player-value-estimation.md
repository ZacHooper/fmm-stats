...
**All 35 changes happen to land on a season-boundary snapshot pair** in this dataset (e.g.
2026-06-29 → 2026-07-02, 2024-06-03 → 2024-06-30) — none observed mid-season. **This is an
observation about our snapshot sample, not a proven mechanism — do not read it as "the game
caches value and refreshes it once a season."** That claim was in an earlier draft of this
section and is retracted: it overreaches what a correlational read of a sparse, import-driven
snapshot schedule can establish. Our snapshots already cluster near season boundaries (that's
when saves tend to get taken), so mid-season pairs in the store are mostly close together in
time — a real trigger that fires every few months, or on a specific in-game event unrelated to
the calendar (a scout report, a match, board confidence, anything), could easily produce this
exact pattern without value being season-locked at all. "Season boundary" may just be a proxy
for "the only gaps in this store wide enough to have caught it." Telling those apart needs
snapshots taken closer together mid-season, which the store doesn't have yet.
...
predict a bigger jump. That test is agnostic to the timing question above and stands regardless.
...
Garly example). What's proven is that some undecoded input drives it; what's NOT established is
when or how often it updates. If that field is ever found, it would be a genuine improvement to
add — contract length and a raw performance metric were both checked here and are not it — but
finding it likely needs denser mid-season snapshots, not more inference from the ones we have.
