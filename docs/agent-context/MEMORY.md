...
- [Remote-agent SQL access](remote-duckdb-access.md) — DONE, PR #1 open awaiting merge: scrubbed DuckDB copy in R2, `ATTACH 's3://...' (READ_ONLY)` over httpfs using R2 creds directly (not the Worker — `*.workers.dev` is often unreachable from a restricted sandbox). Verified against the real store (484,746 rows, ca/pa NULLed). Use `TYPE s3` + explicit `ENDPOINT` (not `TYPE r2`/`ACCOUNT_ID`, which mis-routes), and fetch the `httpfs` extension over HTTPS by hand if `INSTALL` 403s (it defaults to plain HTTP).
